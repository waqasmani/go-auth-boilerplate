...
type Options struct {
	...
	// CookieCSRF configures Origin/Referer validation for the two endpoints
	// that consume the refresh token via an HttpOnly cookie (/auth/refresh and
	// /auth/logout).  TrustedOrigin should match cfg.FrontEndDomain.
...
func New(
	...
	emailmodule.RegisterRoutes(authGroup, emailMod.Handler, jwtHelper, log)
	authmodule.RegisterRoutes(authGroup, authMod.Handler, opts.RateLimit, opts.CookieCSRF)
...
// CookieCSRFConfigFromValues constructs a middleware.CookieCSRFConfig from the
// frontend domain string stored in config — mirrors the other ConfigFromValues
// helpers so app.go never imports the middleware package directly.
func CookieCSRFConfigFromValues(frontEndDomain string) middleware.CookieCSRFConfig {
	return middleware.CookieCSRFConfig{
		TrustedOrigin: frontEndDomain,
	}
}
